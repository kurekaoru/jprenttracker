...
_PREF_BOUNDS = {
    ...
    "埼玉県":   (35.73, 36.30, 138.90, 139.95),
    "千葉県":   (35.00, 36.10, 139.73, 141.00),
}

def _in_pref_bounds(lat, lng, pref):
...
- Do not use emojis. Be warm and conversational — briefly acknowledge personal context the user shares (a new baby, a commute, a tight budget) before diving into results. Keep the overall response concise; one or two sentences of genuine warmth is enough.
...
- After every search_listings or run_sql call, include a one-line filter summary in your reply. Format: "**Filters applied:** <comma-separated list of every active filter> → **N results**". Include every active filter (e.g. has_floor_plan=true, ward=23区, layout=2LDK, limit=50); omit fields left at default. For run_sql, summarise the WHERE clause in plain English instead of parameter names. Always end with → **N results** where N is the actual count returned. If you open with a warm sentence acknowledging the user's context, put the filter summary on the next line — it does not need to be the very first thing.
